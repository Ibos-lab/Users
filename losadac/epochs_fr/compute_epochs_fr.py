...
def get_fr_info(neu: NeuronData, start_sample, end_sample, start_test, end_test):
    ...
    for loc in ["in", "out"]:
        ...
        with warnings.catch_warnings():
            ...
            res[f"d2_c1_{loc}"] = compute_mean(t1on_c1, 200, d2ed)
            res[f"d2_c5_{loc}"] = compute_mean(t1on_c5, 200, d2ed)
            res[f"d2_o1_{loc}"] = compute_mean(t1on_o1, 200, d2ed)
            res[f"d2_o5_{loc}"] = compute_mean(t1on_o5, 200, d2ed)
            # mean fr by sample
            ...
            res[f"d2_o1c1_{loc}"] = compute_mean(t1on_oc["11"], 200, d2ed)
            res[f"d2_o1c5_{loc}"] = compute_mean(t1on_oc["15"], 200, d2ed)
            res[f"d2_o5c1_{loc}"] = compute_mean(t1on_oc["51"], 200, d2ed)
            res[f"d2_o5c5_{loc}"] = compute_mean(t1on_oc["55"], 200, d2ed)
            res[f"d2_n_{loc}"] = compute_mean(t1on_oc["0"], 200, d2ed)
