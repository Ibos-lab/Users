...
def get_cond(test_data):
    # basic conditions
    s_in_11_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==11),
                            test_data['sample pos']==1))[0]
    s_in_15_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==11), 
                            test_data['sample pos']==1))[0]
    s_in_51_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==11), 
                            test_data['sample pos']==1))[0]
    s_in_55_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==11), 
                            test_data['sample pos']==1))[0]

    s_in_11_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==15), 
                            test_data['sample pos']==1))[0]
    s_in_15_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==15), 
                            test_data['sample pos']==1))[0]
    s_in_51_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==15), 
                            test_data['sample pos']==1))[0]
    s_in_55_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==15), 
                            test_data['sample pos']==1))[0]

    s_in_11_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==51), 
                            test_data['sample pos']==1))[0]
    s_in_15_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==51), 
                            test_data['sample pos']==1))[0]
    s_in_51_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==51), 
                            test_data['sample pos']==1))[0]
    s_in_55_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==51), 
                            test_data['sample pos']==1))[0]

    s_in_11_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==55), 
                            test_data['sample pos']==1))[0]
    s_in_15_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==55), 
                            test_data['sample pos']==1))[0]
    s_in_51_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==55), 
                            test_data['sample pos']==1))[0]
    s_in_55_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==55), 
                            test_data['sample pos']==1))[0]

    s_out_11_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==11), 
                            test_data['sample pos']==-1))[0]
    s_out_15_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==11), 
                            test_data['sample pos']==-1))[0]
    s_out_51_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==11), 
                            test_data['sample pos']==-1))[0]
    s_out_55_t_11= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==11), 
                            test_data['sample pos']==-1))[0]

    s_out_11_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==15), 
                            test_data['sample pos']==-1))[0]
    s_out_15_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==15), 
                            test_data['sample pos']==-1))[0]
    s_out_51_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==15), 
                            test_data['sample pos']==-1))[0]
    s_out_55_t_15= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==15), 
                            test_data['sample pos']==-1))[0]

    s_out_11_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==51), 
                            test_data['sample pos']==-1))[0]
    s_out_15_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==51),
                            test_data['sample pos']==-1))[0]
    s_out_51_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==51), 
                            test_data['sample pos']==-1))[0]
    s_out_55_t_51= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==51), 
                            test_data['sample pos']==-1))[0]

    s_out_11_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==11, test_data['test id']==55), 
                            test_data['sample pos']==-1))[0]
    s_out_15_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==15, test_data['test id']==55), 
                            test_data['sample pos']==-1))[0]
    s_out_51_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==51, test_data['test id']==55), 
                            test_data['sample pos']==-1))[0]
    s_out_55_t_55= np.where(np.logical_and(np.logical_and(test_data['sample id']==55, test_data['test id']==55), 
                            test_data['sample pos']==-1))[0]

    trials_s_in_t_11 =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==11), 
                                                 test_data['sample pos']==1))[0]
    trials_s_in_t_15 =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==15), 
                                                 test_data['sample pos']==1))[0]
    trials_s_in_t_51 =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==51), 
                                                 test_data['sample pos']==1))[0]
    trials_s_in_t_55 =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==55), 
                                                 test_data['sample pos']==1))[0]

    trials_s_out_t_11    =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==11),
                                                    test_data['sample pos']==-1))[0]
    trials_s_out_t_15    =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==15), 
                                                    test_data['sample pos']==-1))[0]
    trials_s_out_t_51    =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==51),
                                                    test_data['sample pos']==-1))[0]
    trials_s_out_t_55    =   np.where(np.logical_and(np.logical_and(test_data['sample id']!=0, test_data['test id']==55),
                                                    test_data['sample pos']==-1))[0]

    trials_s_in_t_match     =   np.concatenate([s_in_11_t_11, s_in_15_t_15, s_in_51_t_51, s_in_55_t_55])
    trials_s_in_t_nomatch   =   np.concatenate([s_in_15_t_11, s_in_51_t_11, s_in_55_t_11, 
                                                s_in_11_t_15, s_in_51_t_15, s_in_55_t_15, 
                                                s_in_11_t_51, s_in_15_t_51, s_in_55_t_51, 
                                                s_in_11_t_55, s_in_15_t_55, s_in_51_t_55])


    trials_s_out_t_match     =   np.concatenate([s_out_11_t_11, s_out_15_t_15, s_out_51_t_51, s_out_55_t_55])
    ...
    return {'s in t 11': trials_s_in_t_11,
...
def decode_test_par(dat):
    ...
    for i in range(len(dat)):
        tmp=get_cond(dat[i])  
        cond.append(tmp)
        if np.min(np.concatenate([tmp['s in t 11'].shape, tmp['s in t 15'].shape, tmp['s in t 51'].shape, tmp['s in t 55'].shape]))>30:
            sel_cells.append(i)
            

    num_cells   =   len(sel_cells)
    sel_cells   =   np.array(sel_cells)[np.random.permutation(num_cells)][:250]
...
with open('/envau/work/invibe/USERS/IBOS/data/Riesling/TSCM/OpenEphys/averaged_structures/test/lip_test_avg', 'rb') as handle:
...
if do_lip==1:
    ...
    perf    =   Parallel(n_jobs = 32)(delayed(decode_test_par)(test_lip_avg) for cell in tqdm(range(num_iter)))
...
if do_pfc==1:
    ...
    #     perf.append(r)
    #     r=[]
    perf    =   Parallel(n_jobs = 2)(delayed(decode_test_par)(test_pfc_avg) for cell in tqdm(range(num_iter)))
...
if do_v4==1:
    ...
    for cell in tqdm(range(num_iter)):
        ...
        perf.append(r)
        r=[]
    perf_v4=reorganize_data(perf, num_iter)
